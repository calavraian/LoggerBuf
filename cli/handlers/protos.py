...
def build():
    ...
    # Generate clean facade (__init__.py)
    _generate_facade()
...
def _generate_facade():
    """Generates a clean __init__.py facade for easy importing."""
    init_path = "data_logs/__init__.py"
    lines = [
    ...
    all_exports = ['Event', 'CounterEvent', 'EventType', 'EventStatus', 'CounterType']

    data = registry.get_registry()
    events = data.get("events", {})
    
    for field_name, event_data in events.items():
        msg_name = event_data["message"]
        proto_file = event_data["file"]
        py_file = proto_file.replace(".proto", "_pb2")
        # Ensure we don't duplicate imports if multiple fields use the same message
        if msg_name not in all_exports:
            lines.append(f"from .{py_file} import {msg_name}")
            all_exports.append(msg_name)

    lines.append("")
    lines.append(f"__all__ = {repr(all_exports)}")

    with open(init_path, "w") as f:
        f.write("\n".join(lines) + "\n")
    
    print("Generated Python Facade (data_logs/__init__.py)")

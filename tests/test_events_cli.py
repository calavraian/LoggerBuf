...
import data_logs.event_status_pb2 as event_status_pb2
...
def test_events_cli_add_type(tmp_path, monkeypatch):
    ...
    # Actually, the real proto file is at data_logs/protos/event_status.proto
    ...
    shutil.copy(os.path.join(real_proto_dir, "event_status.proto"), test_proto_dir / "event_status.proto")
    
    monkeypatch.setattr(events, "PROTO_DIR", str(test_proto_dir))
    monkeypatch.setattr(events, "EVENT_STATUS_PROTO", str(test_proto_dir / "event_status.proto"))
    ...
    with open(test_proto_dir / "event_status.proto", "r") as f:
        content = f.read()

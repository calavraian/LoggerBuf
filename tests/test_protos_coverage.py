...
def test_build_schema_validation_error(monkeypatch, tmp_path):
    monkeypatch.setattr("cli.handlers.protos.get_protos_dir", lambda: str(tmp_path))
    monkeypatch.setattr("cli.handlers.protos.get_main_proto", lambda: str(tmp_path / "main_data.proto"))
...
def test_register_event_discovery(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("cli.handlers.protos.get_protos_dir", lambda: str(tmp_path))
    monkeypatch.setattr("cli.handlers.fields._find_file_for_message", lambda m, f=None: str(tmp_path / "mock.proto"))
    monkeypatch.setattr("cli.utils.registry.register_event", lambda f, m, file: 12)
    monkeypatch.setattr("cli.handlers.protos.build", MagicMock())
...
@patch('cli.utils.registry.register_event')
@patch('cli.handlers.fields._find_file_for_message')
